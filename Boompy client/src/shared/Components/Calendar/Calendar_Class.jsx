  ...
  return (
    <Modal isOpen={modalIsOpen} onRequestClose={closeModal} style={{ overlay: { backgroundColor: 'rgba(0, 0, 0, 0.5)' } }}>
      ...
      <div>
        ...
        <div style={{ marginTop: 10 }}>
          <button onClick={assignClass} disabled={!selectedStartTime || !selectedEndTime}>Asignar clases</button>                 
        </div>
      </div>

      <div>
        {availableHoursForDate.length>0&&<p>Horas disponibles:</p>}
        ...
        <div>
          {availableHoursForDate.map(({ _id, startTime, endTime, reserved, cancel }, index) => (
            <div key={index}>
              ...
              {reserved && (
                <div>
                  <button onClick={() => viewReservedClassDetails(startTime, endTime, _id)}></button>
                </div>
              )}

              <div>
                <p onClick={() => cancelClass(_id,startTime)}>x</p>
              </div>
            </div>
          ))}
        </div>
